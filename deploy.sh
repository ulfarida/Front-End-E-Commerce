...
sudo docker build -f "Dockerfile" -t ulfarida/portofolio:fe-$TRAVIS_BUILD_NUMBER  --label "Maintaner Farida Ulfah <ulfah@alterra.id>" .
# push apps image to docker hub
sudo docker push ulfarida/portofolio:fe-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta7--kubeconfig kube/kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=ulfarida/portofolio:fe-${TRAVIS_BUILD_NUMBER}
